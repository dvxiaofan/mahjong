...
describe('联网消息协议', () => {
  ...
  it('拒绝损坏 JSON、未知版本和非法动作', () => {
    ...
    expect(parseClientMessage({
      protocolVersion: 99, requestId: 'v', type: 'hello',
    })).toMatchObject({ success: false, code: 'unsupported-version' });
    expect(parseClientMessage({
      protocolVersion: 1, requestId: 'bad', type: 'submit-action', expectedRevision: 0,
  ...
  it('四个连接获得各自不同的隐私快照', () => {
    ...
    for (const seat of [0, 1, 2, 3] as const) {
      const messages = handleRoomProtocolMessage(target, { connectionId: `c${seat}`, seat }, {
        protocolVersion: 1,
      ...
      expect(snapshot.snapshot.match.game.players.filter((player) => player.visibility === 'self')).toHaveLength(1);
    }
  });
  ...
  it('连接上下文注入座位并返回动作结果和事件批次', () => {
    ...
    const messages = handleRoomProtocolMessage(target, { connectionId: 'dealer', seat: 0 }, {
      protocolVersion: 1,
      requestId: 'submit-1',
      type: 'submit-action',
      expectedRevision: 0,
    ...
    expect(messages[0]).toMatchObject({ type: 'action-result', result: { accepted: true, revision: 1 } });
    expect(messages[1]).toMatchObject({ type: 'event-batch', revision: 1 });
  });

  it('未入座、加入流程和过期动作返回结构化错误', () => {
    const target = room();
    expect(handleRoomProtocolMessage(target, { connectionId: 'anon', seat: null }, {
      protocolVersion: 1, requestId: 'snapshot', type: 'get-snapshot',
    })[0]).toMatchObject({ type: 'error', code: 'not-joined' });
    expect(handleRoomProtocolMessage(target, { connectionId: 'anon', seat: null }, {
      protocolVersion: 1, requestId: 'join', type: 'join-room', roomId: 'protocol-room', displayName: '玩家',
    })[0]).toMatchObject({ type: 'error', code: 'room-lifecycle-required' });

    const action = target.getSnapshot(0).match.game.legalActions[0]!;
    const messages = handleRoomProtocolMessage(target, { connectionId: 'dealer', seat: 0 }, {
      protocolVersion: 1, requestId: 'stale', type: 'submit-action', expectedRevision: 3,
      action: intentFromAction(action),
    });
  ...
  it('下一局消息在错误阶段返回权威房间拒绝', () => {
    const messages = handleRoomProtocolMessage(room(), { connectionId: 'dealer', seat: 0 }, {
      protocolVersion: 1,
