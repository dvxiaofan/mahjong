...
describe('AI 难度策略', () => {
  ...
  it('进阶难度始终选择最高评分弃牌', () => {
    ...
    const decision = decideAiAction(state, 0, createDifficultyAiPolicy('advanced', seededRandom(1)));
    expect(decision?.action).toEqual(decision?.candidates[0]?.action);
  });

  it('休闲难度可稳定选择非最优候选形成差异', () => {
    const state = createGame({ seed: 100 });
    const decision = decideAiAction(state, 0, createDifficultyAiPolicy('casual', () => 0.99));
